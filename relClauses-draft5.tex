...
\usepackage{gb4e}
%
 % what follows prevents the paragraph after a para/subpara header from indenting,
 % and puts the subparagraph number flushleft
...
\newfontfamily{\ipaFont}[Mapping=tex-text,Renderer=ICU,Scale=MatchLowercase]{Charis SIL}
...
\def\vcl#1.{{\scalebox{0.8}{\textsc{(#1)}}}\kern0.5pt\relax}
\def\gc#1{\textsc{#1}}
%\renewcommand{\s}[1]{\sc \MakeLowercase{#1}}
% citation commands
\def\zSp{\hskip0.16667em\relax}
\newcommand{\citepage}[2]{\citeauthor{#1} {(\citeyear{#1}:\zSp{#2})}}
\newcommand{\citepp}[2]{(\citealp{#1}:\zSp{#2})}
\newcommand{\citeA}[1]{\citeauthor{#1} (\citeyear{#1})}
\newcommand{\citeps}[2]{\citeauthor{#1}{'}s {(\citeyear{#1}:\zSp{#2})}}
\newcommand{\cites}[1]{\citeauthor{#1}'s (\citeyear{#1})}
\newcommand{\citeyearpage}[2]{\hbox{(\citeyear{#1}:\zSp{#2})}}
\newcommand{\citeslashpage}[3]{\citeauthor{#1} {(#2/\citeyear{#1}:\zSp{#3})}}
\newcommand{\citeslashyearpage}[3]{\hbox{(#2/\citeyear{#1}:\zSp#3)}}
\newcommand{\citeslashpp}[3]{(\citeauthor{#1}, {#2/\citeyear{#1}:\zSp{#3})}}
\newcommand{\citeslashA}[2]{\citeauthor{#1} (#2/\citeyear{#1})}
\newcommand{\citealppage}[2]{\citealp{#1}:\zSp{#2}}
...
\def\elicited{\text{\llap{$^\diamond$}}}
\def\elicitedNoskip{\text{{$^\diamond$}}}
\def\bad{\text{\llap{${}^*$}}}
...
same name in Lower Fungom. Though the Ethnologue entry for
``Mundabli'' (associated with ISO~693-3 code~[boe])
also includes the dialects spoken in the
villages of Mufu and Buu (see fig.~\ref{lfmap}), 
this chapter only concerns the variety spoken in Mundabli proper
\citep{ethnologue}. Mundabli, Mufu and Buu are referred to
as the Ji cluster in \cite{good:inprep}.
%
...
earlier article. \cite{lovegren:diss}, a grammar of Mungbam,
 was completed during the time that this volume was under editorial review.
...
forms from different noun classes in Mundabli and the Mungbam
dialect of Munken. 
...
\begin{tabular}{lll}
\Hline
Mundabli	&	Munken		&	gloss		\\
\hline
\ci{gb{\=\aw}}	&	\ci{\'u-kp\H{e}}	&	\gl{\cl3.house}	\\
\ci{dz\=\aw}	&	\ci{\'\i-kp\H{e}}	&	\gl{\cl4.houses}	\\
\ci{y\H{\ih}}	&	\ci{\'\i-dz\H{\eh}h\eh} &	\gl{\cl3.eye}	\\
	\ci{y\H{\ih}}	&	\ci{\'a-dz\H{\eh}h\eh}	&	\gl{\cl7a/\cl6.eyes}	\\
\ci{k\=\oo}		&	\ci{\`a-k\ML{\@}f\@}	&	\gl{\cl7/\cl12.bone}	\\
	\ci{k\=\oo}	&	\ci{b\=\i-k\ML{\@}f\@}	&	\gl{\cl8.bones}		\\	% 26-may-2008 jcg tape
\ci{\dzh\v{u}}	&	\ci{\`\i-b\'e}	&	\gl{\cl9.goat}	\\
	\ci{\dzh\H{u}}	&	\ci{\'\i-b\H{e}}	&	\gl{\cl10.goat}		\\
\Hline
\end{tabular}
...
labeled \gl{\vcl a.}, \gl{\vcl b.}, and \gl{\vcl c.}.
%%
...
\gc{p0}, \gc{p1}, \gc{p2}, and \gc{p3} here). Mungbam has one
future tense, while Mundabli has two (glossed \gc{fut1} and \gc{fut2}).
...
\Hline
        &                       &       \gc{pfv}       &       \gc{ipfv}      \\
\hline
\gc{a}     &       \gc{realis}    &       \ci{\`x}             &       \ci{\`x} $+$ \gc{abl}       \\
        &       \gc{irr}       &       \ci{\={x}}           &       \ci{\={x}} $+$ \gc{abl}     \\
\hline
\gc{b}     &       \gc{realis}    &       \ci{\H{x}}           &       \ci{\^x} $+$ \gc{abl}       \\
        &       \gc{irr}       &       \ci{\ML{x}}          &       \ci{\'{x}} $+$ \gc{abl}     \\
\hline
\gc{c}      &       \gc{realis}    &       \ci{\^x}             &       \ci{\^x} $+$ \gc{abl}       \\
        &       \gc{irr}       &       \ci{\'{x}}           &       \ci{\'{x}} $+$ \gc{abl}     \\
\Hline
\end{tabular}
\caption{Verb conjugation in Mungbam. Tone diacritics are interpreted as
follows: \ci{\`x} $=$ L, \ci{\={x}} $=$ M, \ci{\ML{x}} $=$ ML, \ci{\'{x}} $=$ H, \ci{\^{x}} $=$ HL,
\ci{\H{x}} $=$ S(uperhigh). \gc{abl}$=$ `ablaut'.
...
\item	Comitative (preceded by particle glossed~\gc{com})
\item	Dative (optionally preceded by a general-purpose preposition, followed by a particle glossed~\gc{dat})\footnotemark
...
{Transcription} symbols used in the examples are mostly consistent
...
Elicited sentences are indicated by a \elicitedNoskip~symbol in the free
translation. Ungrammatical examples,all of which are elicited
are indicated by an asterisk (with no \elicitedNoskip~symbol).
...
Finally,
\sref{secMungbamFocus} compares the possibilities
...
Adjectives and numerals, when modifying the head noun in a noun phase,\footnote{%
%
Adjectives and numerals may also head noun phrases.  }\/
have a more variable ordering with respect
...
\cl4a-place \gc{1sg.poss} \gc{\cl4}a.\gc{dem.prox} \gc{rel} \cl6a-water \cl6a.\gc{det} \vcl a.stay.\gc{pfv} \gc{cl4.det}		\\
\glt \elicited`This my place where there is water\dots' (Munken)\footnotemark\label{mg01} %104v
\end{exe}
%
\footnotetext{Glossing abbreviations are as follows:\smallskip

\begin{centering}
\begin{tabular}{llll}
\Hline
\vcl a.,\vcl b.,\vcl c.	&	verbal conjugation class	&	\cl1\dots19(a)	&	noun class	\\
\gc{adj}&	adjectivizer			&	\gc{ass}	&	associative	\\
\gc{aug}&	augment				&	\gc{cop}&	copula			\\
\gc{com}	&	comitative		&	\gc{comp}&	complementizer	\\
\gc{consec}&	consecutive			&	\gc{dem}&	demonstrative	\\
\gc{det}&	definite determiner		&	\gc{dist}&	distal	\\
\gc{ds}&	dummy subject			&	\gc{dsf}&	disfluentive (cf.~\citepage{good:2009}{\S4})	\\
\gc{excl}&	exclamatory			&	\gc{foc}&	focus-field form	\\
\gc{frust}&	frustal			&	 \gc{fut, fut1\dots2}&	future	\\
\gc{ideo}&	ideophone			&	&	\\
\gc{ipfv}&	imperfective			&	\gc{irr}&	irrealis	\\
\gc{loc}&	locative			&	\gc{neg}&	negative	\\
\gc{nmlz}&	nominalizer			&	\gc{poss}&	possessive	\\
\gc{prep}&	preposition			&	\gc{prf}&	perfect		\\
\gc{p0\dots3}&	past tense			&	\gc{pst}&	past tense	\\
\gc{prox}&	proximal			&	\gc{pro}&	proform		\\
\gc{red}&	reduplicant			&	\gc{rel}&	relativizer	\\
\gc{subd}&	subordinator			&	\gc{top}&	topic-field form; topic		\\
\gc{vent}&	ventive				&	\gc{vfoc}&	verum focus	\\
\Hline
\end{tabular}
\end{centering}\smallskip

Verbs not glossed as `\gc{ipfv}' are in their perfective forms. Mungbam verbs
not glossed as `\gc{irr}' are in their realis forms.}
%
...
\cl10-fowl \gc{\cl10.det} \gc{rel} \cl10 \vcl a.fall \cl10-red \gc{\cl10.det}	\\
...
which have concordant relativizers,\footnote{%
Cf. for Isu (Ring) \citepp{kiessling:2011}{39,\,passim},
...
\cl9-fowl \cl9.\gc{det} \gc{sbj} \vcl a.peck-\gc{prf} \cl9-fowl \cl9-other	\\
\glt \elicited`The fowl has pecked another fowl.' (Biya)
...
\cl3-year \cl3-\gc{rel} \cl3-house \cl12-book \vcl a.start.\gc{irr} \vcl c.enter.\gc{irr} \gc{prep} Zhoa \gc{\cl3.det}	\\
\glt `The year when the school at Zhoa was opened\dots' (Biya)
\ex \label{mg101c}
\gll w\v{\@} \uline{k\'{\@}-mj\H{\ih}} n\'{\ih} $[$\'\ny-\sh\^{\i} \'n-d\'e \sh\={\i} l\={e}$]$ k\={\@} j\ML{a}	\\
then \cl12-matter \gc{rel} 1sg-\vcl b.want 1sg-\vcl c.talk.\gc{irr} \vcl a.discuss.\gc{irr} \gc{vent.irr} \gc{\cl12.det} thus	\\
...
\begin{tabular}{lllll}
\Hline
\textsc{Variety}	&	\textsc{rel}	&	\gl{some}	&\gl{reason}	&	\gl{own} (associative)\\
\hline
Munken	&	\ci{n\'{\@}}		&	\ci{-l\eh}			&\ci{\'a-mj\H{u}}	&\ci{-\ny\@{m\@}}				\\
Ngun	&	\ci{n\'{\ih}}	&	\ci{-ne}			&\ci{k\'{\@}-mj\H{\ih}}	&					\\	%9/11 22-apr-2010
Biya	&	\ci{\llap-n\ih}		&	\ci{-n\ih}			&\ci{k\'{\@}-mj\H{\ih}}	&\ci{-kj\ih}					\\
Abar	&	\ci{\ny\'\i}		&	\ci{-lehe}			&\ci{k\'{\@}-\ny\H{u}} 	&\ci{-\ny{i}}				\\
...
coincides in segmental form (but
...
Such a grammaticalization scenario is claimed for some Chadic languages \citepp{frajzyngier:1996}{\S11},
...
%
\footnotetext{Interestingly, in Munken, a subordinating particle~\ci{mj\'{u}}, clearly
related to~\ci{\'a-mj\H{u}}, is attested in a type of aversive construction,
translatable as~\gl{lest}.

\ea   \label{exArgMk0500}
\gll    b\'{\@} \dzh\^u b\H{\eh}h{\eh} mj\'{u} \H{\ny}-\sh\'am m\=e \'a t\`\aw\ng=\=a \'a b\={\@} m\={\@}       \\
\cl2 \vcl{c}.go.away \vcl{b}.exit lest \cl6a-blood \cl6a.\gc{det} \gc{neg} \vcl{a}.touch=\gc{prf} \gc{prep} \cl2.\gc{loc.obj} \gc{loc.}at    \\
\glt    `They have moved away lest the blood [of the freshly slaughtered pig] touch them.' (Munken)
\z
%MK130410.011   b@ dZu bEhE mjU J-Sam me a tON-a a b@ m@
%MK130410.011   they have gone out lest the blood touch on them %3r
...
\gc{ds.cop} there \cl1 \vcl b.stay.day.\gc{ipfv} \vcl b.struggle.\gc{ipfv} \vcl a.walk.\gc{ipfv} \gc{ipfv}
\gc{com} \cl8-fly \cl8.\gc{det} \vcl a.disturb \gc{\cl1} \gc{loc.\cl5-}comb \cl5.\gc{det} \gc{loc.}on	\\
\glt `It's there [i.e. that's why] he is constantly struggling with flies disturbing him [landing] on his comb.' (Abar) % 6r 28-apr-2010

%%
\ex	\label{exAbar124rb}
\gll \`a l\H{\eh} \uline{fw\H{e}} \ny\'{\i} $[$\`u \sh\^o fw\^om l\`an{\`\aw} k\={\aw} m\`{\eh} b\`\i-dz\={a}{\ng} b\={\i} \'\i-d\=a{\ng} \`{\i} \ng\={\@}n$]$	\\
\gc{ds} \vcl b.\gc{cop} there \gc{rel} \cl1 \vcl b.stay.day.\gc{ipfv} \vcl b.struggle.\gc{ipfv} \vcl a.walk.\gc{ipfv} \gc{ipfv}
\gc{com} \cl8-fly \cl8.\gc{det} \gc{loc.\cl5-}comb \cl5.\gc{det} \gc{loc.}on	\\
\glt {\elicited`It is there that [i.e. that's why] he is constantly struggling with flies on his comb.' (Abar)} 	%124r
\end{xlist}
\end{exe}
%
%
...
five \gc{rel} \cl2 \vcl a.fall \gc{\cl2.det}	\\
...
\begin{xlist}
\ex     \gll \uline{\`mb\`\oo{\ng}} \ny\'{\i} $[$N\H{a}{\ng} h\H{a} l\Lbi{\ng}$]$ {\`\aw} \`u p\'\i\til{p\H{\i}}        \\
\cl1.cow \gc{rel} N. \gc{p1} \vcl a.look.for \gc{\cl1.det} \cl1 \gc{vfoc}\til{\vcl b.die} \\
\glt \elicited`The cow that Nang looked for died.' (Abar)      \label{mg03a}
\ex     \gll \uline{\`mb\`\oo{\ng}} \ny\'{\i} $[$N\H{a}{\ng} h\H{a} l\Lbi{\ng} \uline{w\`u}$]$ {\`u}{\footnotemark} \`u p\'\i\til{p\H{\i}}     \\
\cl1.cow \gc{rel} N. \gc{p1} \vcl a.look.for \gc{\cl1} \gc{\cl1.det} \cl1 \gc{vfoc}\til{\vcl b.die}  \\
...
\cl12.\gc{dem.prox} \gc{\cl12.det} \gc{cop} \cl12-pan \gc{rel} \cl2 \vcl a.put.\gc{ipfv}
\cl4a-nkwi.bark \cl4a.\gc{det} \gc{prep} \gc{\cl12} \gc{loc.}in \gc{\cl12.det}   \\
\glt \elicited`This is the dish that nkwi bark is put inside of.' (Abar)
\end{exe}
%
...
1sg-\gc{p1} \gc{vfoc}\til{\vcl b.see} \cl3-knife \gc{rel} N. \gc{p1} slaughter \cl10-fowl
\cl10.\gc{det} (\gc{com} \cl3)	\\
...
1sg-\gc{p1} \gc{vfoc}\til{\vcl b.see} \cl5-place \gc{rel} N. \gc{p1} \vcl a.slaughter \cl10-fowl \cl10.\gc{det} (there)	\\
...
\cl1 now \vcl a.cut.vegetables.\gc{ipfv} \vcl a.stay.\gc{ipfv} \cl8-vegetables then=\cl1-\gc{fut} \vcl a.cook.\gc{irr}	\\
...
\'a-b\^u{\ng} \=a-l\'{\aw} \HM{u}{\footnotemark}  m\`{\@}	\\
\cl12-thing \cl2 \vcl c.hang.\gc{ipfv} \vcl b.keep.\gc{ipfv} \cl8-dress \gc{prep} \cl12 \gc{loc.}on
\cl2 \vcl c.hang.\gc{ipfv} \vcl c.keep.\gc{ipfv} \cl12-dress \cl12-some \gc{prep-\cl1.loc.obj} \gc{loc.}on	\\
\glt `\dots a thing they hang dresses on. They're hanging a dress on [it].' (Munken)	%	4-may-2010 1v
%
\footnotetext{
This word is transcribed and glossed \ci{\'u}~\gl{\gc{loc}} in \citepage{lovegren:diss}{407},
though the present gloss and tonal transcription is more consistent with the analysis
and examples in \citepage{lovegren:diss}{\S9.1}. In Munken, locative object pronouns always
bear mid tone, and those beginning with a vowel or glide may coalesce with the preceding
preposition. I.e., \ci{\HM{u} m\`{\@}} is a shorter variant of \ci{\'a w\=u m\`\@}~\gl{\gc{prep}
\gc{\cl1.loc.obj} \gc{loc.}on}. In an earlier version of this paper, \ci{\'u m\`{\@}}
was treated as being a variant of \ci{\'a m\`{\@}}, with the vowel of the preposition
harmonizing to \ci{[u]} following the preceding back vowel. More likely is that
the speaker simply lost track of the noun class of the object of the locative phrase
(\ci{\'a k\=\i\ m\`{\@}}~\gl{\gc{prep} \gc{\cl12.loc.obj} \gc{loc}.on} was intended).
%In Munken, the general-purpose preposition, normally having the form /\kern0.5pt\'a\kern0.5pt/, 
%can assimilate 
%/\kern0.5pt\'u\kern0.5pt/ following a back vowel, when no locative complement is present.
%Cf. \ci{\`\i-w\`u\ng\ \'a m\`\@}~\gl{on the pig}.
%MK130410.016	i-wuN a m@, b@ to ma Ju J-JE b@ wO naNk@ i-wuN a m@
}

\ex	\label{exBiyaWater}
\gll j\`{\eh} \`u \KrnF\ML{\eh} k\'{\@}-n\H{o} k\'{\@}-d\'u-l{\@} \={\ny}-\ny\'{\eh} 	\\
\gc{comp} \cl1 \vcl b.give.\gc{irr} \cl12-thing \cl12-\vcl b.carry.water-\gc{adj} \cl6a-water	\\
...
%
	...
	\cl1-person \gc{rel} \cl1.\gc{fut} \vcl b.come.\gc{irr} then.\cl1 \vcl b.secretly.\gc{irr} \vcl c.call.\gc{irr}
	\vcl b.exit.\gc{irr} \vcl a.{go}.\gc{irr} \cl5-name	\\
	\glt `The person who comes and whispers [to me] the name\dots' (Ngun) %9-jul 2v
\z
	\ea	\label{biya01a}
	\gll \uline{b\`{\@}-mb\`{\aw}\ng{\=\@}} m\'{\@} n\'{\@} $[$N\H{a}{\ng} ts\H{a}m$]$ b\=e {p\'\i-\H{a}}	\\
	\cl2-cow \gc{1sg.poss} \gc{rel} N. \vcl b.beat \gc{\cl2.det} \vcl b.die	\\
...
\z
        ...
        \cl3-knife \gc{top} \gc{2sg} {\vcl a.do.housework} \gc{com} \gc{\cl3} \cl3-house \cl3-\vcl a.cook.\gc{adj} \gc{loc}     \\
        \glt `That knife that you work with in the kitchen' (Munken)  %       MK 04-05-2010 2V        00:03:18 tape 2
\z

	\ea     \label{munken10}

        \gll \uline{\`u-n\`{\eh}} n\={\@} $[$m\={\@} l\=e f\H{\eh} \'u-{\sh}\H{\eh} \uline{w\`u}=n\={\@}$]$ w{\`\@}={\'\@}
        f\H{\@} gb\`e   \\
        \cl1-person \gc{rel} \gc{1sg} \gc{p2/3} \vcl b.give \cl3-knife \gc{\cl1=dat} \cl1=then
        \gc{p1} \vcl a.fall \\
...
\ea	\label{missong01a}
	...
	\cl1.\gc{nmlz}-\vcl a.fall-\gc{nmlz} \gc{rel} N. \gc{p1} \vcl a.fall \cl1 \gc{loc.}at \gc{\cl1.det} \gc{p1} \gc{vfoc}\til{\vcl c.bad}	\\
...
\ea 	\label{abar01a}
	\gll {u}-w\`{\@}n\`{\aw} \`u l\H{\eh} \uline{\`u-n\`o} {\ny}\={\i} $[$\'n={\dzh}\=a \=\i-{\sh}\ML{\eh} \uline{j\`{\i}}$]$       \\
        \cl1-\gc{dem.prox} \cl1 \vcl b.\gc{cop} \cl1-person \gc{rel} \gc{1sg}=\vcl a.steal.\gc{irr} \cl9-fowl \gc{3sg.poss}     \\
...
\gc{1sg}-\gc{vfoc}\til{know} \cl12-matter \gc{rel} N. \gc{p1} \vcl a.fight \gc{com} \cl10-pig \gc{\cl10.det} \gc{\cl12.det}  \\
\glt \elicited`I know the reason that Nang was fighting with the pigs.'
(Munken) %p.105
\ex \label{mg06aa}
\gll \=m-b\^{\aw}f{\@} \'a-mj\H{u} \'u-kp\HM{\aw}f{\@} m\'{\@}	\\
\gc{1sg-}\vcl b.ask.\gc{ipfv} \cl12-matter \cl3-money \gc{1sg.poss}	\\
...
relation of the head nominal within the relative
clause itself is unclear, as in~(\ref{mg06}). 
%The analysis of
%this type of construction is discussed in~\sref{secMungbamFormal}.
%
\begin{exe}
\ex \gll \`\i-j\={\@}n{\@} \uline{\`\i-\ny\ML{\i}} n\'{\@} $[$\`u-n\`{\eh} \ny\`{\eh} \`u ts\ML{\@}n$]$	\\
\cl5-\gc{dem.prox} \cl5-honey \gc{rel} \cl1-person \vcl a.stay.\gc{pfv} \cl1 \vcl b.be.drunk.\gc{irr}	\\
...
\cl5-sound \cl5-\gc{rel} \cl2 \vcl a.cut.\gc{ipfv} \vcl a.stay.\gc{ipfv} \cl12-tree
\gc{\cl12.det} \vcl a.make.\gc{ipfv} \vcl b.block.\gc{ipfv} \vcl a.stay.\gc{ipfv} \gc{1sg}	\\
...
%today \gc{cop} what \gc{rel} \gc{2sg} come.\gc{irr} home thus	\\
...
\cl12-matter \gc{rel} \cl2-children \gc{\cl2.det} \vcl a.steal \gc{\cl9-fowl} \gc{1sg.poss} \gc{\cl12.det} \vcl b.worry \gc{1sg}	\\
...
\cl2 \vcl b.have \cl5-species \cl3-house \cl2 \vcl b.have \cl3-money	\\
...
relative clauses. 
Mungbam may be such a language, given the
presence of relative clauses which would have
to be analyzed, in an extraction analysis, as being derived from
an {\it ungrammatical}\/ sentence in a main clause.
...
N. \gc{p1} \vcl a.climb \cl3-house \gc{\cl3.det} \gc{ prep} \gc{loc}.top		\\
\glt \elicited`Nang climbed *(on top of) the house.' (Munken)	\label{mg05b}
\z
\ea	\gll N\H{a}{\ng} f\H{\@} b\Lbi{m}\til{b\ML{a}n}	\\
N. \gc{p1} \gc{vfoc}\til{\vcl a.climb}	\\
...
1sg-\gc{p1} \vcl b.see \cl3-house \gc{rel} N. \gc{p1} \vcl a.climb \gc{prep} \cl3 \gc{loc}.top \gc{\cl3.det}	\\
...
\cl5-\gc{dem.prox} \gc{neg} \cl5-\vcl b.tired.\gc{inf} \cl5-other N. \gc{p1} \vcl b.tired \gc{frust}	\\
\glt \elicited`This is not the fatigue that Nang was tired [i.e. that Nang experienced].'  (\gc{p1}, Biya) %78r
%
\ex	\label{exMGtense02}
\gll \v{a} \uline{\`u-n\`o} \ny\'{\i} $[$M\^{u} k\`{\@} l\`e t\^{\oo} \`\i-bw\'{e} K\'ul\'{\aw}=n\'e$]$	\\
\gc{ds.cop} \cl1-person \gc{rel} M. \gc{p2/3}{\footnotemark} \vcl a.make \vcl c.show \cl9-goat K.=\gc{dat}	\\
\glt \elicited`This is the person$_i$ by whom$_i$ Mu made the goat be shown to Kulo.' (\gc{p2}, Abar)	%	2r 5-jul-2000
\ex	\label{exMGtense03}
\gll \KrnF\ML{\eh} w\`u b=\uline{\'u-t\H{\aw}f{\@}} n\'{\@}m\={\aw} w\={\@}, \`u n\'{\@} $[$\`a l\=e \KrnF\ML{\eh} d\=a$]$ w\={\@} \\
 \vcl b.give.\gc{irr} \cl1 \gc{com=}\cl3-sense \gc{top} \gc{\cl3.det} \cl1 \gc{rel} \gc{2sg}
\gc{p2/3} \vcl b.give.\gc{irr} before \gc{\cl3.det}	\\ % 2r 26-jul-2010 (check this tone?)
\glt `Give him that wisdom, the one which you have given before [to others].' (\gc{p3}, Munken) 
\end{exe}
%
\footnotetext{The tense markers for \gc{p2} and \gc{p3} are identical in all Mungbam varieties
except for in Biya. The tenses are distinguished by the fact that in \gc{p3} the verb
...
Formal focus-marking processes
in Mungbam include focalization and defocalization 
of clausal arguments (realized by
word order changes) and verum focus (realized by reduplication 
of the final verb in the verbal complex).\footnote{%
%
A more detailed treatment of focus in Mungbam is found in
\citepage{lovegren:diss}{\S11}. The relevant notion of focus
assumed here is that of \cite{hyman:1984}.
}\/
...
\cl1-person \gc{rel} \gc{ds} \vcl a.fall \cl1 \gc{\cl1.det} \cl1 \gc{vfoc}\til{\vcl b.die}	\\
...
\cl1-person \gc{rel} \gc{ds} \vcl c.insult N. \gc{\cl1} \gc{\cl1.det} \cl1 \gc{vfoc}\til{\vcl b.die}	\\
...
\cl1-person \gc{rel} N. \gc{\cl1} \gc{neg} \vcl a.insult.\gc{irr} \gc{neg2} \gc{\cl1.det} \cl1 \gc{vfoc}\til{\vcl b.die}	\\
...
\begin{xlist}
\ex	\gll N\H{a}{\ng} \'a b\'am h\={\aw}	\\
N. \gc{neg} \vcl c.ascend.\gc{irr} \gc{neg}	\\
\glt \elicited`Nang did not accept.' (Abar) %19
...
N. \vcl a.slaughter \cl10-goat \cl10.\gc{det} \cl16-\gc{loc.}stream	\\
\glt \elicited`Nang killed the goats at the stream.' (Biya) 	%73v
\end{xlist}
\ex \label{mg1048}
\begin{xlist}
\ex \label{mg1048a}
\ci{\bad \`a gb\`u{\til}{gb\ML{e}} N\H{a}{\ng}	}
\ex \label{mg1048b}
...
\ex	\label{mg104a}
\gll \uline{\`mb\`\oo{\ng}} \ny\'{\i} $[$N\H{a}{\ng} h\H{a} \{l\Lbi\ng~/~l\Lbi{n}\til{l\MLbi\ng}\}$]$ \`{\aw} \`u p\'\i{\til}p\H{\i}	\\
\cl1.cow \gc{rel} N. \gc{p1} (\vcl a.look~/~\gc{vfoc}\til{\vcl a.look}) \gc{\cl1.det} \cl1 \gc{vfoc}\til{\vcl b.die}	\\
\glt \elicited`The cow that Nang \{looked for~/~did look for\} died.' (Abar)
\ex	\label{mg104b}
\gll \uline{\`mb\`\oo{\ng}} \ny\'{\i} $[$N\H{a}{\ng} h\H{a} \{l\Lbi\ng~/~*l\Lbi{n}\til{l\MLbi\ng}\} \uline{w\`u}$]$ \`{u}{\footnotemark} \`u p\'\i{\til}p\H{\i}	\\
\cl1.cow \gc{rel} N. \gc{p1} \{\vcl a.look~/~\gc{vfoc}\til{\vcl a.look}\} \gc{\cl1} \gc{\cl1.det} \cl1 \gc{vfoc}\til{\vcl b.die}	\\
...
\cl10-\gc{10.dem} \gc{\cl10.det} \cl10-pig \gc{1sg.poss} \cl10-\gc{rel} N.  \gc{p1} \vcl b.beat \cl10-three \gc{\cl10.det} 	\\
\glt \elicited`These are my pigs that Nang beat three [of them].' (Biya) % 9v 2012
\ex \label{by1012-mc}
\gll N\H{a}{\ng}  {\'\i-g\H{\aw}{\ng}} m\H{\@} \`a \tch\H{a}m \'\i-t\={\eh}	\\
N. \cl10-pig \gc{1sg.poss} \gc{ds} \vcl b.beat \cl10-three	\\
...
\cl1-person \cl1-\gc{rel} \cl1 \vcl a.fall \gc{\cl1.det} \cl1 \gc{vfoc}\til{\vcl b.die}	\\
\glt \elicited`The man who fell died.' (Biya)	% 86r
\end{exe}
%%
...
\begin{exe}
	...
	\ex \gll 	\uline{ŋwàtɨ̀} bi̋ bī-fyɨ̋ŋ b-ɛ́n bi̋-tɔ᷇ b-ɔ́ nō̤ [wù fə̌ ta̋ŋ b-ɔ́ Bàmɛ́ndà]		\\
			\cl7/8.book \cl8.\gc{3sg.poss} \cl8-new \cl8-\gc{dem.prox} \cl8-three \cl8-\gc{det} \gc{subd} \cl1  \gc{p1} \vcl b.buy \cl8-\gc{rel} B. 		\\
...
\begin{exe}
	\ex \label{exSheSaidAyi} 		
		...
		\cl1  \vcl b.say	\gc{excl} \gc{1sg.top}=\gc{fut1}	\vcl c.go.up	there		\cl3/7a.attic 	
		\gc{1sg.top}=\vcl a.go	\vcl a.see clearly	\cl7.thing	\cl7	\vcl a.make.\gc{ipfv}	\vcl c.extinguish.\gc{ipfv} 	\vcl b.move.away.\gc{ipfv} \cl7-\gc{rel}	\cl3/7a.fire	\cl3-\gc{det} \\
...
%		\gc{1sg.top}=(\gc{a})love 7/8.clothes 7-{det} \gc{rel} (\gc{b})buy 7-\gc{det} name 7 	\\
...
\begin{exe}
	\ex \label{exTheFirstThing} 		
		...
		first	\cl7.thing	\gc{subd} \gc{1sg.top}=\gc{fut2}	\vcl a.do	\cl7-\gc{rel}	\cl7 	\vcl b.be	 \gc{comp}	{\dots} \\
		\glt \VSP \gl{The first thing I will do, is: [\dots]})
\end{exe}%Mundabli-01-Apr -2008-RMV-12.002
%\begin{exe}
%	\ex \label{exYouHaveSeenTheThings} 		
%		\gll à də̄ \uline{ndʒɔ́m} [\uline{bī} fɨ̋ b-ɔ́ ā wà] \\
%			\gc{2sg.top.pvb}	(\gc{a})see	8.things	8 	(\gc{b})pass	8-\gc{det}	\gc{com}	\gc{2sg.foc.npvb} \\
...
\begin{exe}
	\ex \label{exThisChildYouAre} 		
		\gll wān w-ɛ̄n, dɨ̌ \uline{wà} nō̤ [\uline{à} lə̄ w-ɔ̄ ná mɨ̄ wān w-ɔ̄ lɛ̄ fán gbɔ̄ kúŋ]\\
			\cl1.child	\cl1-\gc{dem.prox} \vcl b.be	\gc{2sg.foc}	\gc{subd} \gc{2sg.top} \vcl a.make \cl1-\gc{rel}	  as	\gc{1sg.foc} \cl1.child	\cl1-\gc{det}	\vcl a.get.lost.\gc{ipfv}	here \cl3.house	behind \\
...
\footnotetext{The phrase \ci{mɨ̄ wān} \gl{my child} is a fixed lexicalized expression. While possessive phrases are usually head-initial, consisting of a head noun followed by a possessive pronoun which agrees with the noun class of the head nominal, in this fixed expression, the noun \gl{child} is simply juxtaposed to the focus form of the 1\gc{sg} pronoun.}
...
%			Wum \gc{loc}.village \gc{subd} \cl1  (\gc{b})be \cl1-\gc{rel} \cl3/7.village \gc{advlz} \cl19-small , \cl7  \gc{neg}1 be.far \gc{neg}2 \\
...
%			1\gc{pl} (\gc{a})search 9.goat 9.certain  \gc{rel}	 9  \gc{p1} (\gc{c})run 9-\gc{det}						\\
...
%			1\gc{pl} (\gc{a})search 9.goat \gc{rel}	 9  \gc{p1} (\gc{c})run 9-\gc{det}						\\
...
%			1\gc{pl} (\gc{a})search 9.goat 9-\gc{det} \gc{rel}	 9  \gc{p1} (\gc{c})run 9-\gc{det}						\\
...
\begin{exe} 
	\ex \label{exGoatTheyKilled1}	
		\gll	\uline{dʒǔ} nō̤ [bə̄ kə̀ lə̌ kpɨ̄ y-ɔ̄ (\uline{yì}) tō b-ɔ́ ŋgɔ᷆] kə̀ bān áná būbūbūbū			\\
			\cl9.goat \gc{subd} \gc{impers.pro} \gc{p3} \vcl a.make \vcl b.die \cl9-\gc{rel} \cl9 \cl7/8.day \cl8-\gc{det} upon \gc{p3} \vcl b.be.white like.that \gc{ideo}.white		\\
...
\begin{exe} 
	\ex \label{exThenFromThen}	
		\gll	then from then, mɨ̄ m=fə̋ kì-yùŋn{\`\ih} bɔ̌ gbàm lā nō̤ wù fə̋ ná kpɒ̋ ŋgɔ́ w-ɔ́ ndá lā			\\
			then from then \gc{1sg.foc} \gc{1sg.top}=\vcl b.give \cl7-thanks also \cl7/8.god \gc{dat} \gc{subd} \cl1 \vcl b.give  as \cl3/7a.money upon \cl3-\gc{det} \gc{1sg.dat} \gc{dat}		\\
...
%			\cl1  (\gc{c})sing (\gc{b})good (\gc{b})pass (\gc{b})pass as \cl1  (\gc{a})play as2		\\
...
%			\cl2.children \cl2-\gc{det} (\gc{b})hear \gc{subd} \cl2-mother \gc{3pl.poss} \gc{p3} (\gc{b})say as		\\
...
\cl1.chief \cl1.\gc{ass} 3s \vcl a.eat.\gc{ipfv.p1.dsf} \cl1.\gc{det} \cl14.fufu	\\
...
\begin{exe}
	\ex \label{exThePersonWhoWasPreaching2} 		
		...
		\cl1.man	\cl1 	\gc{p3}	\vcl b.say.\gc{ipfv}	\cl1-\gc{rel}	\cl3a.word \cl7/8.god	\cl7/8.day	\cl7-\gc{det}	 upon	\gc{p3}	\vcl b.be	Pa P. K. \\
...
\begin{exe} 
	\ex \label{exGoatTheyKilled2ResPro}	
		...
		\gll	\uline{dʒǔ} nō̤ [bə̄ kə̀ lə̌ kpɨ̄ y-ɔ̄ (\uline{yì}) tō b-ɔ́ ŋgɔ᷆] kə̀ bān áná būbūbūbū			\\
			\cl9.goat \gc{subd} \gc{impers.pro} \gc{p1} \vcl a.make \vcl b.die \cl9-\gc{rel} \cl9  \cl7/8.day \cl8-\gc{det} upon \gc{p3} \vcl b.be.white like.that \gc{ideo}.white		\\
		\glt \VSP \elicited \gl{The goat which was killed on that day was completely white.}
%\end{xlist}
\end{exe}%Fieldnotes2012/1/p46(Elicitation)
...
\begin{exe}
	\ex \label{exThePotIntoWhich2}	
		...
		\cl3.pot \cl3-\gc{det} \gc{subd} \cl2  \gc{p1} \vcl a.put \vcl c.enter \vcl a.go.down \cl3-\gc{rel} \cl3a.beans \cl3a-\gc{det} \cl3  in \cl3  \gc{p1} \vcl b.be.big \gc{advlz} very 	\\
...
postverbal relativizer agrees with a locative proform glossed \gl{\gc{prox}}, likely
...
\begin{exe}
	\ex \label{exWhenYouHaveFinishedSquishing} 		
		...
		when	\gc{2sg.top}	\vcl a.squish	\vcl a.finish		when	\gc{1sg.top}=\vcl b.drink	\gc{1sg.foc}	\gc{1sg.top}=\vcl a.show	\cl9/10.place	\gc{subd}			\cl1.child		\cl1-\gc{2sg.poss}	\gc{p3}	\vcl b.pass	\gc{prox}-\gc{rel}	\\
...
%\begin{exe}
	\ex \label{exThePeopleForWhomWithPro}	
		...
		\cl2.people \gc{subd} \cl1  \gc{p3} \vcl b.cook.\gc{ipfv} \cl2-\gc{rel} (\gc{loc}) \cl2.\gc{loc}  \gc{dat} \cl2  \gc{p3} \vcl a.be.rich	\\
...
%\begin{exe}
	\ex \label{exThePeopleForWhomOnlyPP}
		...
		\cl2.people \gc{subd} \cl1  \gc{p3} \vcl b.cook.\gc{ipfv} \cl2-\gc{rel} \gc{dat} \cl2  \gc{p3} \vcl a.be.rich	\\
		\glt \VSP \elicited \gl{The people for whom she used to cook were rich.}
%\end{exe}%phone elicitation 26/06/2012
...
%		\cl2.people \gc{subd} \cl1  \gc{p3} (\gc{b})cook.\gc{ipfv} \cl2-\gc{rel} \gc{loc} \gc{dat} \cl2  \gc{p3} (\gc{a})be.rich	\\
...
%		\cl9.knife \gc{rel} \cl1  \gc{p1} (\gc{b})cut \cl9-\gc{det} \cl9.meat \cl9-\gc{det} \gc{com} \cl9 \cl9 \gc{p1} (\gc{a})be.blunt	\\
...
%%\begin{exe}
	\ex \label{exTheKnifeThatSheCut}	
		...
		\cl9.knife \gc{subd} \cl1  \gc{p1} \vcl b.cut \cl9-\gc{rel} \cl9.meat \cl9-\gc{det} \cl9  \gc{p1} \vcl a.be.blunt	\\
...
%		\cl9.knife \gc{subd} \cl1  \gc{p1} (\gc{b})cut \cl9-\gc{det} \cl9.meat \cl9-\gc{det} \gc{com}, \cl9  \gc{p1} (\gc{a})be.blunt	\\
...
%		\gc{1sg.top}=(\gc{b})ask QUOT.Q Q , (\gc{b})be what \gc{rel} \gc{2sg.top} (\gc{b})refuse 7-\gc{det} 6a.wine 6a-\gc{det} \gc{2sg.foc}-N \gc{loc}.hands Q.\gc{quot} \\
...
%		\cl1  ?.take (\gc{b})come ?.basket , \cl1  \gc{consec} ?.put \cl?.cassava (\gc{loc}) in	\\
...
%		\gc{1sg.top}=(\gc{a})love 7/8.clothes 7-\gc{det} \gc{rel} Nyungfu (\gc{b})buy=7-\gc{det} 7 	\\
...
\begin{exe}
	\ex \label{exThePersonWhoWasPreaching1} 		
		\gll \uline{mɔ̀} [\uline{wù} kə̀ dzé w-ɔ̄ dʒu᷇ gbàm tō k-ɔ́ ŋgɔ᷆] kə̀ dɨ̄ Pǎ Pi̋tà Kìá\\
		\cl1.man	\cl1 	\gc{p3}	\vcl b.say.\gc{ipfv}	\cl1-\gc{rel}	\cl3a.word \cl7/8.god	\cl7/8.day	\cl7-\gc{dist.dem}	 upon	\gc{p3}	\vcl b.be	Pa P. K. \\
		\glt \VSP \gl{The person who was preaching on that day was Pa Peter Kia.}
\end{exe}%Mundabli-01-Apr -2008-RMV-10.004%same example is used above!%mention that it's repeated or chose different example!
...
\begin{exe}
	\ex \label{exChildWhomIGaveMoney}	
		...
		\gll	\uline{wàn} w-ɔ̄ [nō̤ m=fə̋ fə̋ w-ɔ̄ kpɒ̋ {\H\ih} \uline{wú} lā] kɛ́ tʃǔ a̋ kè-kè	\\
			\cl1.child \cl1-\gc{det} \gc{subd} \gc{1sg.top}=\gc{p1} \vcl b.give	\cl1-\gc{rel} \cl3/7a.money \gc{loc} \cl1.\gc{loc} \gc{dat} \vcl c.return \vcl b.come \gc{advlz} \cl9.hand\til{\gc{red}} 				\\
...
\begin{exe}
	\ex \label{exTheHoeWithWhich}	
		\gll	\uline{ŋkɔ̀m} [nō̤ ntí fə̋ lɔ̄ w-ɔ̄ ā \uline{wù}] kwa᷆ fɨ̌  	\\
			\cl1/2.hoe \gc{subd} N. \gc{p1} \vcl a.go.bush	\cl1-\gc{rel} \gc{com} \cl1 \vcl a.break \vcl b.pass 				\\
...
\begin{exe}
	\ex \label{exThePotIntoWhich}	
		...
		\cl3.pot \cl3-\gc{det} \gc{subd} \cl2  \gc{p1} \vcl a.put \vcl c.enter \vcl a.go.down \cl3-\gc{rel} \cl3a.beans \cl3a-\gc{det} in \cl3  \gc{p1} \vcl b.be.big \gc{advlz} very 	\\
		\glt \VSP \elicited \gl{The pot into which they put the beans was very big.}
\end{exe}%fieldnotes2012/p.50	%???
...
\begin{exe} 
	\ex \label{exChildWhoseName}	
		...
		\gll	\uline{wān} [nō̤ mán \uline{mű} dɨ̋ w-ɔ̄ ngàʃǎ] fə̋ lɔ̀ ɲu᷇				\\
			\cl1.child \gc{subd} \cl11/12.name \cl12.\gc{3sg.poss} \vcl b.be \cl1-\gc{rel} N. \gc{p1} \vcl a.go.bush \cl3/7a.farm		\\
...
%		7  (\gc{b})be only 7/8.day 6a.corn.beer 9(?).die 8-\gc{det} \gc{rel} prop.name \gc{fut2} (\gc{b})come 8-\gc{det} with 9.goat	\\
...
%		\cl1-\gc{det} \cl1.time \gc{rel} \gc{1sg.top}=(\gc{a})take only \cl1-\gc{rel} \cl8.things \cl8.\gc{1sg.poss} now		\\
...
\begin{exe}
	\ex \label{exTheSoundThatThey}	
		\gll 	dzɛ̋ nō̤ bə̄ gbā ná kpān w-ɔ́ fyá mɨ̄ ā fa̋m	\\
			\cl3/7a.sound \gc{subd} \gc{impers.pro} \vcl a.cut.\gc{ipfv} as \cl3.tree \cl3-\gc{det} \vcl b.give.\gc{ipfv} \gc{1sg.foc} \gc{com} \cl7/8.problem	\\
...
%\comment{check \gc{fut1} in relative clauses! All other tenses are expressed in the same way. (see notebook1/2012/p.93) Expressed differently from main clause? Possible to do the same but more commonly different.}
%
...
\begin{exe} 
	\ex \label{exTheManWhoBuiltWhose}	
		...
		\cl1.man \gc{subd} \cl1  \vcl c.build \cl1-\gc{rel} \cl3.house who \vcl b.die	\\
...
\begin{exe}
	\ex \label{exTheKnifeTheyCutWhat}	
		...
		\cl9-this \vcl b.be \cl9.knife \gc{subd} \gc{impers.pro} \gc{p1} \vcl b.cut \cl9-\gc{rel} what \gc{com} \cl9 	\\
...
\begin{exe}
	\ex \label{exWeLookFor}	
		...
		\gll 	b{\=\ih} tsè \uline{mɔ̀} [nō̤ fə̋ gīa w-ɔ̄ \uline{wù} dʒǔ y-ɔ̄]		\\
			1\gc{pl} \vcl a.search \cl1.man \gc{subd} \gc{p1} \vcl a.steal \cl1-\gc{rel} \cl1  \cl9.goat \cl9-\gc{det}\\
...
\begin{exe}
	\ex \label{exILikeTheClothesThatNyungfu}	
		...
		\gc{1sg.top}=\vcl a.love \cl7/8.clothes \cl7-\gc{det} \gc{subd} \vcl b.buy \cl7-\gc{rel} N. \cl7 	\\
...
\begin{exe}
	\ex \label{exILikeTheClothes}	
		...
		\gc{1sg.top}=\vcl a.love \cl7/8.clothes \cl7-\gc{det} \gc{subd} Nyungfu \vcl b.buy \cl7-\gc{rel} \cl7 	\\
...
\begin{exe}
	\ex \label{exHereIsTheMan}	
		...
		\gll 	fán dɨ̋ \uline{mɔ̀} [nō̤ \uline{wù} tə́ kə̀ tʃū w-ɔ̄]	\\
			here \vcl b.be \cl1.man \gc{subd} \cl1  \gc{vfoc} \gc{p3} \vcl b.come \cl1-\gc{rel}\\
...
%		 	\cl7/8.day	certain	\cl8.suffering 	catch	\cl8-\gc{rel} \cl1 	\cl7/8.thousand \cl8-those	\cl8-five	\gc{neg}1 (\gc{b})be	\gc{neg}2	\\
...
\begin{exe} 
	\ex \label{exTheyAreNotListening}	
		...
		\gll 	bī ā wṳ́ wɔ̄ ɲɔ̄ ni̋	\\
		 	\cl8  \gc{neg} \vcl b.hear.\gc{ipfv} 	\gc{neg} \vcl b.talk	\cl1.mother.\gc{3sg.poss} \\
...
\begin{exe} 
	\ex \label{exIHaveNotTurnedFufu}	
		...
		\gll 	n=da̋ ɲɔ̋m wɔ̄ ɲa᷆m	\\
		 	\gc{1sg.top}=\vcl b.be.\gc{neg} \vcl b.turn.fufu  \gc{neg} \cl14.fufu	\\
		\glt \VSP \elicited \gl{I have not turned fufu yet.}
\end{exe}%

%XXX\comment{da̋ = dɨ̋ a̋ ? = (\gc{b})be still? DONAT}
...
%\cl1-person \gc{rel} \gc{ds} \gc{neg} N. \cl1.\gc{top} (\gc{a})insult \cl1.\gc{foc} \gc{frust} \cl1.\gc{top} \gc{vfoc}\til{die}	\\
...
\begin{exe} 
	\ex \label{exTheyTookTheBag}	
		...
		\gll 	bɔ̋ fə̋ mù \uline{bɔ̀} k-ɔ́ [nō̤ b{\=\ih} fə̋ dɨ̋ k-ɔ́ a̋ də̀ wɔ̄], bɔ̋ mɨ́ dʒì kǐ {\H\ih} kìa mɨ̄.	\\
		 	\cl2  \gc{p1} \vcl a.take	\cl7/8.bag \cl7-\gc{det} \gc{subd} \gc{1pl} \gc{fut1} \vcl b.be \cl7-\gc{rel} \gc{neg} \vcl a.see \gc{neg} \cl2  \gc{consec} \vcl a.put \cl7  \gc{loc} \cl9.basket in \\
...
\gc{aug$_1$}=\cl1-thief 1(\gc{pst})-steal-\gc{fv} \gc{aug$_9$}=\cl9-elephant \cl9-of his	\\
\glt ``The thief stole his elephant.''
%%
\ex 
\gll e=mw-{idz\uline{'}} y\'a-\uline{i}b-a n-dovu \phantom{xx} ha-\textrtailt\uline{\'{a}}w-a	\\
\gc{aug$_1$}=\cl1-thief \cl1(\gc{rel-pst})-steal-\gc{fv} \cl9-elephant \phantom{xx} \cl1(\gc{pst})-run.away-\gc{fv}	\\
...
%%the situation described by the \gc{rc}.'' \citepp{andrews:2007}{206}	\label{defAndrews}
%%
%\end{xlist}
%\end{exe}
%
...
%\begin{xlist}
%\ex	\gll \uline{\`mb\`\oo{\ng}} \ny\'{\i} $[$N\H{a}{\ng} h\H{a} l\Lbi{\ng}$]$ {\`\aw} \`u p\'\i\til{p\H{\i}}	\\
%\cl1.cow \gc{rel} N. \gc{p1} look.for \gc{\cl1.det} \cl1 \gc{vfoc}\til{die}	\\
%\glt \elicited``The cow that Nang looked for died.'' (Abar)	\label{mg03a}
%\ex	\gll \uline{\`mb\`\oo{\ng}} \ny\'{\i} $[$N\H{a}{\ng} h\H{a} l\Lbi{\ng} \uline{w\`u}$]$ {\`u} \`u p\'\i\til{p\H{\i}}	\\
%\cl1.cow \gc{rel} N. \gc{p1} look.for \gc{\cl1.obj} \gc{\cl1.det} \cl1 \gc{vfoc}\til{die}	\\
%\glt \elicited``The cow that Nang looked for [it] died.'' (Abar)	\label{mg03b}
%\end{xlist}
%\end{exe}
%%%
...
%\ex \label{mg06a}
%\gll \=n-ts\Lbi{ts\LM{a}} \uline{\'a-mj\H{u}} n\'{\@} $[$N\H{a}{\ng} f\H{\@} \ny\`\oo{n\@} b\={\@} \'\i-w\H{\aw}{\ng} j\={\eh}$]$ k\={\@}	\\
%1sg-\gc{vfoc}\til{know} \cl7-matter \gc{rel} N. \gc{p1} fight \gc{comit} \cl10-pig \gc{\cl10.det} \gc{\cl7.det}	\\
...
%1sg-\gc{p1} see N. \cl1 come.\gc{ipfv} \gc{comit=\cl3-}rope	\\
%\glt \elicited``I saw Nang coming with (i.e. bringing) a rope.'' (Munken)	\label{mg0200a}
%\ex 	\gll \=m-f\H{\@} ts\H{\aw}{\ng} \uline{\'u-gb\H{\eh}} n\'{\@} $[$N\H{a}{\ng} f\H{\@}
%t\^o *(b\={\@} \uline{w\`u})$]$	\\
%1sg-\gc{p1} see \cl3-rope \gc{rel} N. \gc{p1} come *(\gc{comit} \cl3)	\\
...
%\cl1.cow \gc{rel} N. \gc{p1} (look~/~\gc{vfoc}\til{look}) \gc{\cl1} \gc{\cl1.det} \cl1 \gc{vfoc}\til{die}     \\
%\glt \elicited``The cow$_i$ that Nang (looked for~/~*did look for) it$_i$ died.'' (Abar) %      81
%\end{exe}
%%
%\begin{exe}
...
%\gc{ds} \gc{cop} \cl10-fowl \gc{\cl10.det} N.  slaughter 	\\
